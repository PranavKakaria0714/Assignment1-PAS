...
[cite_start]This project implements a non-linear data transformation and parameter estimation pipeline as required by Assignment-1[cite: 1]. The objective is to model the probability density of a transformed air quality feature (`NO2`) using a specific exponential function parameterized by a university roll number.

## Dataset
* [cite_start]**Source:** India Air Quality Data [cite: 4]
* [cite_start]**Feature Used:** `NO2` (Nitrogen Dioxide) [cite: 3]
* **File:** `city_day.csv`
...
[cite_start]The transformation logic is[cite: 6, 8]:
$$z = x + a_r \sin(b_r x)$$

[cite_start]Where coefficients $a_r$ and $b_r$ are derived from the University Roll Number ($r$) as follows[cite: 9, 10]:
* $$a_r = 0.05 \times (r \pmod 7)$$
* $$b_r = 0.3 \times ((r \pmod 5) + 1)$$

### 2. Parameter Estimation
We fit a specific probability density function (PDF) to the distribution of the transformed variable $z$. [cite_start]The target model is given by[cite: 12]:
...
## Dependencies
* `numpy`: Matrix operations and math functions.
* `pandas`: Data loading and handling.
* `scipy`: Optimization (`curve_fit`) for parameter learning.
* `sklearn`: Metric calculation (`r2_score`).
* `matplotlib`: Data visualization.

## Usage
1.  Place `city_day.csv` in the root directory.
2.  Update the `ROLL_NUM` variable in the script.
3.  Run the script:
    ```bash
    python solution.py
    ```
4.  [cite_start]Submit the values of $\lambda$, $\mu$, and $c$ to the provided form[cite: 15].
